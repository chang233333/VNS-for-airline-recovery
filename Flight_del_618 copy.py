...
class Param:
    ...
    def _prepare_passenger_costs(self):
        f = self.df
        self.c1pj = {}
        cost_dict = dict(zip(f.P1.astype(int), f.P1_cost.astype(int)))
        max_p = max(cost_dict.keys()) if cost_dict else 0
        cost_p = np.array([cost_dict.get(i, 0) for i in range(1, max_p + 1)], dtype=int)
        for p_idx, p in enumerate(range(1, max_p + 1)):
            for j_idx, j in enumerate(self.L):
                key = f"{p}_{j}"
                self.c1pj[key] = int(cost_p[p_idx] * f['FT'].values[j_idx])
        self.c2pj = self.c1pj.copy()
...
if __name__ == '__main__':
    ...
    CON = param.CON  # 直接拿到 CON 矩阵（pandas.DataFrame
    #dis,dis_time,ind_dis,dis_value = 3,600,"FRA","1100-1300"
    #dis,dis_time,ind_dis,dis_value = 2,600,3,0
    #dis,dis_time,ind_dis,dis_value = 1,600,16,60
